    ...
    if (existingAccessToken || existingRefreshToken) {
      try {
        ...
        return res.status(400).json({
          message:
            "You are already logged in. Please logout first to login with a different account.",
...
export const saveUserProfile = async (req, res) => {
  ...
  const { collegeName, branch, studyType, phone, gender, role, dob, features } =
    req.body;
...
export const updateFeatures = async (req, res) => {
  ...
  if (!features || typeof features !== "object")
    return res.status(400).json({ message: "Invalid features object" });
  try {
    const user = await User.findById(userId);
